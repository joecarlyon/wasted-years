...
function findMatchingRecipe(beerName, recipes) {
  ...
  match = recipes.find((r) => {
    ...
    return (
      normalizedRecipe.includes(normalizedBeer) ||
      normalizedBeer.includes(normalizedRecipe)
    )
  })
  if (match) return match
...
function getCategory(style) {
  ...
  if (styleLower.includes('ipa') || styleLower.includes('pale ale'))
    return 'ale'
  if (styleLower.includes('stout') || styleLower.includes('porter'))
    return 'ale'
  if (
    styleLower.includes('wheat') ||
    styleLower.includes('hef') ||
    styleLower.includes('weiss') ||
    styleLower.includes('wit')
  )
    return 'ale'
  if (
    ...
    return 'lager'
  if (styleLower.includes('saison') || styleLower.includes('belgian'))
    return 'ale'
...
function parseHop(hopStr) {
  ...
  if (match) {
    ...
    let usage = 'Both'
    if (timing.includes('boil') || timing.includes('bitter'))
      usage = 'Bittering'
...
function main() {
  ...
  for (const row of csvRows) {
    ...
    const abv =
      parseFloat(row['ABV']) || (recipe ? parseFloat(recipe.abv) || 0 : 0)
  ...
  const arrayMatch = batchesContent.match(
    /export const batches: Batch\[\] = (\[[\s\S]*\])\s*$/
  )
  if (!arrayMatch) {
...
`
  ...
  console.log(
    `Brewfather batches: #${newBatches.length + 1}-${allBatches.length}`
  )
}
