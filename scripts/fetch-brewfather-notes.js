...
async function main() {
  ...
  for (const batch of batchList) {
    console.log(
      `Fetching details for batch #${batch.batchNo}: ${batch.name || batch.recipe?.name || 'Unknown'}...`
    )
    try {
  ...
  for (const batch of batchesWithNotes) {
    ...
    if (batch.fermentationNotes)
      console.log(`  Fermentation Notes: ${batch.fermentationNotes}`)
    if (batch.bottlingNotes)
      console.log(`  Bottling Notes: ${batch.bottlingNotes}`)
  }
