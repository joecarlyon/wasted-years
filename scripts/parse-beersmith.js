...
const OUTPUT_PATH = '/Users/joecarlyon/dev/wasted-years/data/beersmith-recipes.json';
...
function parseBsmx(filePath) {
  ...
  const style = decodeHtmlEntities(styleSection ? getTag(styleSection[1], 'F_S_NAME') : '');
...
function fuzzyMatch(recipeName, csvEntries) {
  ...
  const normalizedRecipe = decodedName.toLowerCase()
    .replace(/[^a-z0-9]/g, '')
    .replace(/og$/, '')
    .replace(/3$/, '')
    .replace(/2$/, '')
  ...
  for (const entry of csvEntries) {
    ...
    if (normalizedRecipe.includes(normalizedCsv) ||
  ...
  for (const [key, csvName] of Object.entries(partialMatches)) {
    if (normalizedRecipe.includes(key) || key.includes(normalizedRecipe)) {
      const match = csvEntries.find(e =>
        e.Beer.toLowerCase().replace(/[^a-z0-9]/g, '').includes(
          csvName.toLowerCase().replace(/[^a-z0-9]/g, '')
        )
      );
...
function main() {
  ...
  // Get all .bsmx files
  const files = fs.readdirSync(BEERSMITH_DIR)
    ...
    try {
      ...
      if (recipe && recipe.name) {
        ...
        if (csvMatch) {
          ...
          if (csvMatch['ABV'] && csvMatch['ABV'] !== '' && csvMatch['ABV'] !== '0') {
  ...
  console.log(`Recipes with notes: ${recipes.filter(r => r.tonyNotes || r.joeNotes).length}`);
}
